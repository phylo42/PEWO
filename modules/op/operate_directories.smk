'''
prepare directorires which corresponds to paramters combination tested in the pipeline
...
#rule all:
#    input: expand(config["workdir"]+"/RAPPAS/{pruning}/k{k}_o{omega}",pruning=range(0,config["pruning_count"],1),k=k_list(),omega=omega_list())
...
rule build_rappas_exp_dir:
    ...
    output:
        directory(config["workdir"]+"/RAPPAS/{pruning}/k{k}_o{omega}")
    version: "1.00"
    run:
        #for k in k_list():
        ...
        if (wildcards.k in k_list()) and (wildcards.omega in omega_list()):
            dir=config["workdir"]+"/RAPPAS/"+str(wildcards.pruning)+"/k"+str(wildcards.k)+"_o"+str(wildcards.omega)
            if not os.path.exists(dir):
                os.mkdir(dir)
        else:
            print("k/omega combinations not defined in config")