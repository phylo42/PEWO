    ...
    run:
        if config["config_epang"]["premask"]==1:
            shell(
                ...
                epa-ng --redo --preserve-rooting on --filter-max {params.maxp} --filter-min-lwr {params.minlwr} --baseball-heur --verbose -w {params.tmpdir} -q {input.q} -t {input.t} --ref-msa {input.r} -T 1 -m {input.m} &> {log}
            ...
            shell(
                """
                mkdir -p {params.tmpdir}
                epa-ng --redo --no-pre-mask --preserve-rooting on --filter-max {params.maxp} --filter-min-lwr {params.minlwr} --baseball-heur --verbose -w {params.tmpdir} -q {input.q} -t {input.t} --ref-msa {input.r} -T 1 -m {input.m} &> {log}
                cp {params.tmpdir}/epa_info.log {params.dir}/{wildcards.pruning}_r{wildcards.length}_h3_epang_info.log
                cp {params.tmpdir}/epa_result.jplace {params.dir}/{wildcards.pruning}_r{wildcards.length}_h3_epang.jplace
                rm -r {params.tmpdir}
                """
