...
rule placement_epang_h1:
    ...
    output:
        os.path.join(config["workdir"],"EPANG")+"/{pruning}/h1/{pruning}_r{length}_h1_g{gepang}_epang.jplace"
    log:
        config["workdir"]+"/logs/placement_epang/{pruning}_r{length}_h1_g{gepang}.log"
    benchmark:
        repeat(config["workdir"]+"/benchmarks/{pruning}_r{length}_h1_g{gepang}_epang_benchmark.tsv", config["repeats"])
    version: "1.0"
    params:
    ...
    run:
        if config["config_epang"]["premask"]==1:
            shell(
                ...
                epa-ng --redo --preserve-rooting on --filter-max {params.maxp} --filter-min-lwr {params.minlwr} -g {wildcards.gepang} --verbose -w {params.tmpdir} -q {input.q} -t {input.t} --ref-msa {input.r} -T 1 -m {input.m} &> {log}
            ...
            shell(
                """
                mkdir -p {params.tmpdir}
                epa-ng --redo --no-pre-mask --preserve-rooting on --filter-max {params.maxp} --filter-min-lwr {params.minlwr} -g {wildcards.gepang} --verbose -w {params.tmpdir} -q {input.q} -t {input.t} --ref-msa {input.r} -T 1 -m {input.m} &> {log}
                cp {params.tmpdir}/epa_info.log {params.dir}/{wildcards.pruning}_r{wildcards.length}_h1_g{wildcards.gepang}_epang_info.log
                cp {params.tmpdir}/epa_result.jplace {params.dir}/{wildcards.pruning}_r{wildcards.length}_h1_g{wildcards.gepang}_epang.jplace
                rm -r {params.tmpdir}
                """
