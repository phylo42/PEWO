...
rule placement_appspam:
    ...
    shell:
        """
        appspam -s {input.s} -q {input.q} -t {input.t} -m {wildcards.matchingmode} -g {wildcards.assignmentmode} -k {wildcards.d} -o {output.jplace} --threshold {wildcards.filteringthreshold} --histogram >& {log}
        """